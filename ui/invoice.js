...
// Note: The following shipments are overdesigned for a specific customer and need to be fixed for general use
    ...
    //                      return d.item.quantity + ' @ ' + d.item.unit_amount_display;
            ...
            if( s == 'items' ) {
                if( j == 0 ) {
                    ...
                    if( d.item.code != null && d.item.code != '' ) {
                        return '<span class="maintext">' + d.item.code + ' - ' + d.item.description + '</span>' + (d.item.notes!=null&&d.item.notes!=''?'<span class="subtext">'+d.item.notes.replace(/\n/g, '<br/>')+'</span>':'');
                    }
                    if( d.item.notes != null && d.item.notes != '' ) {
                        return '<span class="maintext">' + d.item.description + '</span><span class="subtext">' + d.item.notes.replace(/\n/g, '<br/>') + '</span>';
                    }
        ...
        this.invoice.rowFn = function(s, i, d) {
            ...
            if( s == 'transactions' ) {
                if( d != null && d.transaction != null && d.transaction.id > 0 ) {
                    return 'M.ciniki_sapos_invoice.transaction.open(\'M.ciniki_sapos_invoice.showInvoice();\',\'' + d.transaction.id + '\',0);';
                } 
                return '';
...
//                    },
                ...
                'manufacturing_status':{'label':'Manufacturing', 'visible':'yes', 'type':'select', 'options':M.ciniki_sapos_invoice.manufacturingStatuses},
                ...
                'preorder_status':{'label':'Pre-Order Status', 'type':'select', 
                    'visible':function() { return 'no'; return M.modFlagSet('ciniki.sapos', 0x400000); },
...
//            this.donationToggle();
        ...
        this.item.updateDonation = function() {
            ...
            if( (f&0x8800) == 0x0800 ) {
                this.sections.details.fields.unit_donation_amount.visible = 'yes';
            }
            if( M.modFlagOn('ciniki.sapos', 0x08000000) ) {
...
/*                p.sections.details.fields.flags1.on_fields = [];
    ...
    this.transaction.sections = {
        ...
        '_buttons':{'label':'', 'buttons':{
            'save':{'label':'Save', 'fn':'M.ciniki_sapos_invoice.transaction.save();'},
            'delete':{'label':'Delete', 
                'visible':function() { return M.ciniki_sapos_invoice.transaction.transaction_id > 0 ? 'yes' : 'no';},
                'fn':'M.ciniki_sapos_invoice.transaction.remove(M.ciniki_sapos_invoice.transaction.transaction_id);',
                },
            }},
    };
    ...
    this.transaction.save = function() {
        if( this.transaction_id > 0 ) {
            ...
            var src = this.formValue('source');
            M.api.postJSONCb('ciniki.sapos.transactionAdd', {'tnid':M.curTenantID,
                'invoice_id':this.invoice_id}, c, function(rsp) {
                    if( rsp.stat != 'ok' ) {
                        M.api.err(rsp);
                        return false;
                    }
                    if( M.curTenant.sapos.settings['transaction-etransfer-email'] != null 
                        && M.curTenant.sapos.settings['transaction-etransfer-email'] == 'yes' 
                        && src == 110
                        ) {
                        M.ciniki_sapos_invoice.email.open('M.ciniki_sapos_invoice.showInvoice();',M.ciniki_sapos_invoice.invoice.data);
                    } else {
                        M.ciniki_sapos_invoice.transaction.close();
                    }
                });
    ...
    this.transaction.addButton('save', 'Save', 'M.ciniki_sapos_invoice.transaction.save();');
    ...
    this.start = function(cb, aP, aG) {
        ...
        if( M.modFlagOn('ciniki.sapos', 0x20000000) ) {
            ...
            this.shippingStatuses['20'] = 'Instore Pickup';
            this.shippingStatuses['55'] = 'Ready for Pickup';
            this.shippingStatuses['60'] = 'Picked Up';
        ...
        this.paymentStatuses['40'] = 'Deposit';
        this.paymentStatuses['50'] = 'Paid';
        this.paymentStatuses['55'] = 'Refund Required';
        this.paymentStatuses['60'] = 'Refunded';
...
//          this.invoice.sections.details.list.invoice_type_text.visible = 'yes';
        ...
        //
        // Setup the donation flags
        //
        this.item.sections.details.fields.flags1 = {'label':'Donation', 
            'type':'flagtoggle', 'field':'flags', 'bit':0x8000, 'default':'off', 
            'active':'no',
            'onchange':'M.ciniki_sapos_invoice.item.updateDonation',
            };
        if( M.modFlagSet('ciniki.sapos', 0x06000000) == 'yes' ) {
            this.item.sections.details.fields.flags1 = {'label':'Donation', 
                'type':'flagspiece', 'field':'flags', 'mask':0x8800, 
                'toggle':'yes', 'join':'yes', 'none':'yes',
                'flags':{'12':{'name':'Partial'},'16':{'name':'Full Amount'}},
                'onchange':'M.ciniki_sapos_invoice.item.updateDonation',
                'active':'yes',
                };
        } 
...
//        }
        ...
        p.sections.shipitems.addTxt = (rsp.invoice.status < 45)?'Add':'';
        p.sections.items.addTxt = (rsp.invoice.status < 45||(M.curTenant.sapos.settings['rules-invoice-paid-change-items'] != null && M.curTenant.sapos.settings['rules-invoice-paid-change-items'] == 'yes'))?'Add':'';
...
//        }
/* REMOVED - OLD SHIPMENTS CODE, needs a rewrite to be good for general use
