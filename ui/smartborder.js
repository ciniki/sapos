...
function ciniki_sapos_smartborder() {
    ...
    this.shipments.sections = {
        'datepicker':{'label':'', 'type':'datepicker', 
            ...
            'fn':'M.ciniki_sapos_smartborder.shipments.showDay',
            'hint':'Search',
        ...
        'shipments':{'label':'Shipments', 'type':'simplegrid', 'num_cols':9,
            'headerValues':['Ship Date', 'Customer', '# Boxes', 'Weight', '# Pieces', 'Notes', 'Total', 'Value', 'Avg'],
            'headerClasses':['', '', 'alignright', 'alignright', 'alignright', 'alignright', 'alignright', 'alignright', 'alignright'],
            'sortable':'yes',
            'sortTypes':['date','text','number','number','number','text','number','number','number'],
            'cellClasses':['', '', 'alignright', 'alignright', 'alignright', 'alignright', 'alignright', 'alignright', 'alignright'],
            'noData':'No shipments',
            },
        };
    ...
    this.shipments.cellValue = function(s, i, j, d) {
        switch(j) {
            ...
            case 7: return d.shipment.total_value_display;
            case 8: return d.shipment.total_value_avg_display;
        }
    };
    this.shipments.rowFn = function(s, i, d) {
        if( s == 'shipments' ) {
            return 'M.startApp(\'ciniki.sapos.invoice\',null,\'M.ciniki_sapos_smartborder.shipments.open();\',\'mc\',{\'invoice_id\':\'' + d.shipment.invoice_id + '\'});';
        }
    };
    this.shipments.open = function(cb, dt) {
        if( dt != null ) { this.date = dt; }
        M.api.getJSONCb('ciniki.sapos.reportSmartBorder', 
            {'business_id':M.curBusinessID, 'start_date':this.date}, function(rsp) {
                if( rsp.stat != 'ok' ) {
                    M.api.err(rsp);
                    return false;
                }
    ...
    this.shipments.showDay = function(i, dt) {
        this.open(null, dt);
    };
    ...
    this.start = function(cb, appPrefix, aG) {
        ...
        this.cb = cb;
        if( args.date != null ) {
            this.shipments.open(cb, args.date);
        } else {
            this.shipments.open(cb, null);
        }
    }
