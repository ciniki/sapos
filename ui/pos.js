...
function ciniki_sapos_pos() {
    ...
    this.checkout.sections = {
        ...
        'customer_details':{'label':'', 'aside':'yes', 'type':'simplegrid', 'num_cols':2,
            'cellClasses':['label',''],
            'addTxt':'Edit',
            'addFn':'M.startApp(\'ciniki.customers.edit\',null,\'M.ciniki_sapos_pos.checkout.open();\',\'mc\',{\'next\':\'M.ciniki_sapos_pos.checkout.updateCustomer\',\'customer_id\':M.ciniki_sapos_pos.checkout.data.customer_id});',
            'changeTxt':'Change customer',
            'changeFn':'M.startApp(\'ciniki.customers.edit\',null,\'M.ciniki_sapos_pos.checkout.open();\',\'mc\',{\'next\':\'M.ciniki_sapos_pos.checkout.updateCustomer\',\'customer_id\':0});',
            },
...
//            'terminal':{'label':'Process Payment', 'fn':'M.startApp(\'ciniki.sapos.terminal\',null,\'M.ciniki_sapos_pos.checkout.open();\',\'mc\',{\'detailsFn\':M.ciniki_sapos_pos.terminalDetails});'},
//            'submitorder':{'label':'Submit Order', 'fn':'M.ciniki_sapos_pos.submitOrder(M.ciniki_sapos_pos.invoice.invoice_id);'},
//            'applydiscount':{'label':'Apply Discount', 'fn':'M.ciniki_sapos_pos.applyDiscount(M.ciniki_sapos_pos.invoice.invoice_id);'},
//            'picklist':{'label':'Print Pick/Pack', 'fn':'M.ciniki_sapos_pos.printPickList(M.ciniki_sapos_pos.invoice.invoice_id);'},
...
//                  case 4: return d.tenant_amount;
    ...
    this.item.save = function() {
        if( this.item_id > 0 ) {
            ...
            M.api.postJSONCb('ciniki.sapos.invoiceItemAdd', {'tnid':M.curTenantID, 'invoice_id':this.invoice_id}, c, function(rsp) {
                if( rsp.stat != 'ok' ) {
                    M.api.err(rsp);
                    return false;
                }
    ...
    this.item.remove = function() {
        ...
        if( confirm("Are you sure you want to remove this item?") ) {
            M.api.getJSONCb('ciniki.sapos.invoiceItemDelete', {'tnid':M.curTenantID, 'item_id':this.item_id}, function(rsp) {
                if( rsp.stat != 'ok' ) {
                    M.api.err(rsp);
                    return false;
                }
    ...
    this.transaction.sections = {
        'details':{'label':'', 'fields':{
//            'transaction_type':{'label':'Type', 'type':'toggle', 'default':'20', 'toggles':M.ciniki_sapos_pos.transactionTypes},
...
/*    this.edit = new M.panel('Invoice',
        'ciniki_sapos_pos', 'edit',
        'mc', 'medium mediumaside', 'sectioned', 'ciniki.sapos.invoice.edit');
    this.edit.invoice_id = 0;
    this.edit.data = {};
    this.edit.sections = {
        'details':{'label':'', 'aside':'yes', 'fields':{
            'invoice_type':{'label':'Type', 'active':'yes', 'type':'toggle', 'default':'invoice', 'toggles':M.ciniki_sapos_pos.invoiceTypes, 'fn':'M.ciniki_sapos_pos.edit.updateForm();'},
...
//                    'active':function() { return M.modFlagSet('ciniki.sapos', 0x02000000); },
//                    },
            'status':{'label':'Status', 'type':'select', 'options':M.ciniki_sapos_pos.invoiceStatuses},
            'payment_status':{'label':'Payment', 'active':'yes', 'type':'select', 'options':M.ciniki_sapos_pos.paymentStatuses},
            'shipping_status':{'label':'Shipping', 'active':'yes', 'type':'select', 'options':M.ciniki_sapos_pos.shippingStatuses},
            'manufacturing_status':{'label':'Manufacturing', 'active':'yes', 'type':'select', 'options':M.ciniki_sapos_pos.manufacturingStatuses},
            'donationreceipt_status':{'label':'Donation Receipt', 'active':'yes', 'type':'select', 
                'active':function() { M.ciniki_sapos_pos.edit.data.donation != null && M.ciniki_sapos_pos.edit.data.donation == 'yes' ? 'yes' : 'no'},
                'options':M.ciniki_sapos_pos.donationreceiptStatuses},
        ...
        '_buttons':{'label':'', 'buttons':{
            'save':{'label':'Save', 'fn':'M.ciniki_sapos_pos.saveInvoice();'},
            }},
    };
    ...
    this.edit.addButton('save', 'Save', 'M.ciniki_sapos_pos.saveInvoice();');
...
//              'description':{'label':'Description', 'type':'text', 'livesearch':'yes'},
        ...
        '_buttons':{'label':'', 'buttons':{
            'save':{'label':'Save', 'fn':'M.ciniki_sapos_pos.saveItem();'},
            'delete':{'label':'Delete', 'fn':'M.ciniki_sapos_pos.deleteItem(M.ciniki_sapos_pos.item.item_id);'},
            }},
        };
    this.item.liveSearchCb = function(s, i, v) {
        this.liveSearchRN++;
        ...
        if( i == 'code' || i == 'description' ) {
            M.api.getJSONBgCb('ciniki.sapos.invoiceItemSearch', {'tnid':M.curTenantID,
                'field':i, 'invoice_id':M.ciniki_sapos_pos.invoice.invoice_id, 'start_needle':v, 'limit':15}, function(rsp) {
                    if( sN == M.ciniki_sapos_pos.item.liveSearchRN ) {
                        M.ciniki_sapos_pos.item.liveSearchShow(s,i,M.gE(M.ciniki_sapos_pos.item.panelUID + '_' + i), rsp.items);
                    }
                });
    ...
    this.item.liveSearchResultClass = function(s,f,i,j,d) {
        if( d.price_description != null && d.price_description != '' 
            && this.sections[s].fields[f].livesearchcols-j == 1 ) {
            return 'multiline';
        }
        return 'multiline';
    ...
    this.item.liveSearchResultValue = function(s,f,i,j,d) {
        if( j == 0 ) {
            var mt = d.description;
            if( (f == 'code' || f == 'description') && d != null ) { 
                if( d.code != null && d.code != '' ) {
                    mt = d.code + ' - ' + d.description;
                }
            }
            if( M.curTenant.sapos.settings != null && M.curTenant.sapos.settings['quote-notes-product-synopsis'] == 'yes' ) {
                d.notes = d.synopsis;
            ...
            if( d.notes != '' ) {
                return '<span class="maintext">' + mt + '</span><span class="subtext">' + d.notes + '</span>';
            }
            return mt;
        }
        if( j == 1 && this.sections[s].fields[f].livesearchcols == 3 ) {
            if( d.available_display != null && d.available_display != '' ) { return d.available_display; }
            if( d.registrations_available != null ) { return d.registrations_available; }
            if( d.inventory_available != null ) { return d.inventory_available; }
        }
        if( this.sections[s].fields[f].livesearchcols-j == 1 ) {
            if( d.price_description != null && d.price_description != '' ) {
                return '<span class="maintext">' + d.unit_amount + '</span><span class="subtext">' + d.price_description + '</span>';
            }
            return d.unit_amount;
        }
        return '';
    };
    this.item.liveSearchResultRowStyle = function(s,f,i,d) {
        if( d.inventory_available <= 0 ) {
            return 'background: ' + M.ciniki_sapos_pos.colours['invoice-item-backordered'] + ';';
        }
        return '';
    };
    this.item.liveSearchResultRowFn = function(s,f,i,j,d) {
        if( (f == 'code' || f == 'description') && d != null ) {
            return 'M.ciniki_sapos_pos.item.updateFromSearch(\'' + s + '\',\'' + f + '\',\'' + d.object + '\',\'' + d.object_id + '\',\'' + escape(d.code!=null?d.code:'') + '\',\'' + escape(d.description) + '\',\'' + d.quantity + '\',\'' + escape(d.unit_amount) + '\',\'' + escape(d.unit_discount_amount) + '\',\'' + escape(d.unit_discount_percentage) + '\',\'' + d.taxtype_id + '\',\'' + d.price_id + '\',\'' + d.flags + '\',\'' + escape(d.notes) + '\');';
        }
    };
    ...
    this.item.addButton('save', 'Save', 'M.ciniki_sapos_pos.saveItem();');
    this.item.addClose('Cancel');
    ...
    this.transaction.sections = {
        'details':{'label':'', 'fields':{
            ...
            'transaction_date':{'label':'Date', 'type':'text', 'size':'medium'},
            'source':{'label':'Source', 'type':'select', 'options':M.ciniki_sapos_pos.transactionSources},
        ...
        '_buttons':{'label':'', 'buttons':{
            'save':{'label':'Save', 'fn':'M.ciniki_sapos_pos.saveTransaction();'},
            'delete':{'label':'Delete', 'fn':'M.ciniki_sapos_pos.deleteTransaction(M.ciniki_sapos_pos.transaction.transaction_id);'},
            }},
    };
    this.transaction.fieldValue = function(s, i, d) {
        if( this.data != null && this.data[i] != null ) { return this.data[i]; }
        ...
        if( args != null && args.invoice_id != null && args.invoice_id > 0 && objects != null ) {
            M.api.postJSONCb('ciniki.sapos.invoiceObjectsAdd', {'tnid':M.curTenantID,
                'invoice_id':args.invoice_id, 'customer_id':cid}, c, function(rsp) {
                    ...
                    var p = M.ciniki_sapos_pos.invoice;
                    p.invoice_id = args.invoice_id;
                    M.ciniki_sapos_pos.showInvoiceFinish(cb, rsp);
                });
        } else {
...
//          p.sections.details.list.status_text.visible = 'yes';
        ...
        if( rsp.invoice.status < 50 || M.curTenant.permissions.owners != null ) {
            p.addButton('edit', 'Edit', 'M.ciniki_sapos_pos.editInvoice(\'M.ciniki_sapos_pos.showInvoice();\',M.ciniki_sapos_pos.invoice.invoice_id);');
        }
        if( rsp.invoice.invoice_type == '10' ) {
            if( p.rightbuttons['add'] == null ) {
                this.invoice.addButton('add', 'Invoice', 'M.ciniki_sapos_pos.createInvoice(M.ciniki_sapos_pos.invoice.cb,0,null);');
            }
        ...
        if( cid != null && this.invoice.data.customer_id != cid ) {
            ...
            M.api.getJSONCb('ciniki.sapos.invoiceUpdate', {'tnid':M.curTenantID,
                ...
                'billing_update':'yes', 'shipping_update':'yes', 'work_update':'yes'}, function(rsp) {
                    ...
                    M.ciniki_sapos_pos.showInvoiceFinish(null,rsp);
                });
        } else {
            M.api.getJSONCb('ciniki.sapos.invoiceUpdate', {'tnid':M.curTenantID,
...
//          this.showInvoice();
    ...
    this.updateInvoiceField = function(f, n) {
        var v = prompt(n, M.ciniki_sapos_pos.invoice.data[f]);
        if( v != null && v != '' ) {
            ...
            M.api.postJSONCb('ciniki.sapos.invoiceUpdate', {'tnid':M.curTenantID,
                'invoice_id':this.invoice.invoice_id}, c, function(rsp) {
                    ...
                    M.ciniki_sapos_pos.showInvoice();
                });
        }
    };
        ...
        if( confirm("Are you sure you want to remove this invoice from the system?") ) {
            M.api.getJSONCb('ciniki.sapos.invoiceDelete', {'tnid':M.curTenantID,
                'invoice_id':iid}, function(rsp) {
                    ...
                    M.ciniki_sapos_pos.invoice.close();
                });
        }
    };
