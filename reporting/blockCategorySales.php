...
function ciniki_sapos_reporting_blockCategorySales(&$ciniki, $tnid, $args) {
    ...
    if( isset($args['category']) && $args['category'] != '0' ) {
        $category_sql = "AND items.category = '" . ciniki_core_dbQuote($ciniki, $args['category']) . "' ";
    }
...
//        . "DATE_FORMAT(IFNULL(transactions.transaction_date, invoices.invoice_date), '" . ciniki_core_dbQuote($ciniki, $date_format) . "') AS invoice_date, "
        ...
        . "LEFT JOIN ciniki_sapos_transactions AS transactions ON ("
            ...
            . "AND transactions.transaction_type < 60 "    // No refunds listed here
        ...
        . "AND invoices.invoice_date >= '" . ciniki_core_dbQuote($ciniki, $start_dt->format('Y-m-d')) . "' "
        ...
        . "ORDER BY category, invoice_date, items.invoice_id, items.id, customers.display_name "
        . "";
