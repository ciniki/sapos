...
// <rsp stat='ok' id='34' />
//
function ciniki_sapos_templates_order(&$ciniki, $business_id, $invoice_id, $business_details, $sapos_settings, $output='download') {
    //
    ...
    //
    $w = array(60, 20, 20, 20, 40, 20);
    $pdf->SetFillColor(224);
    $pdf->SetFont('', 'B');
    $pdf->SetCellPadding(2);
    $pdf->Cell($w[0], 6, 'Item', 1, 0, 'C', 1);
    $pdf->Cell($w[1], 6, 'Quantity', 1, 0, 'C', 1);
    $pdf->Cell($w[2], 6, 'Shipped', 1, 0, 'C', 1);
    $pdf->Cell($w[3], 6, 'Backorder', 1, 0, 'C', 1);
    $pdf->Cell($w[4], 6, 'Price', 1, 0, 'C', 1);
    $pdf->Cell($w[5], 6, 'Total', 1, 0, 'C', 1);
    $pdf->Ln();
...
//      $pdf->Cell($w[0], $lh, $item['item']['description'], 1, 0, 'L', $fill, '', 0, false, 'T', 'T');
        ...
        $pdf->MultiCell($w[1], $lh, $item['item']['quantity'], 1, 'R', $fill, 
            0, '', '', true, 0, false, true, 0, 'T', false);
        $pdf->MultiCell($w[2], $lh, $item['item']['shipped_quantity'], 1, 'R', $fill, 
            0, '', '', true, 0, false, true, 0, 'T', false);
        $pdf->MultiCell($w[3], $lh, $item['item']['quantity']-$item['item']['shipped_quantity'], 1, 'R', $fill, 
            0, '', '', true, 0, false, true, 0, 'T', false);
        if( ($invoice['flags']&0x01) > 0 ) {
            $pdf->MultiCell($w[1], $lh, $item['item']['unit_discounted_amount_display'], 1, 'R', $fill, 
                0, '', '', true, 0, false, true, 0, 'T', false);
        } else if( $discount == '' ) {
//          $pdf->Cell($w[1], $lh, $quantity . $item['item']['unit_amount_display'], 1, 0, 'R', $fill, '', 0, false, 'T', 'T');
            $pdf->MultiCell($w[4], $lh, $item['item']['unit_amount_display'], 1, 'R', $fill, 
                0, '', '', true, 0, false, true, 0, 'T', false);
        } else {
            $pdf->MultiCell($w[4], $lh, $item['item']['unit_amount_display'] . (($discount!='')?"\n" . $discount:""), 1, 'R', $fill, 0, '', '', true, 0, false, true, 0, 'T', false);
        }
//      $pdf->Cell($w[2], $lh, $item['item']['total_amount_display'], 1, 0, 'R', $fill, '', 0, false, 'T', 'T');
        $pdf->MultiCell($w[5], $lh, $item['item']['total_amount_display'], 1, 'R', $fill, 
            0, '', '', true, 0, false, true, 0, 'T', false);
        $pdf->Ln(); 
        $fill=!$fill;
    }
