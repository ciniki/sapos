...
// -------
//
function ciniki_sapos_wng_chequeCheckout(&$ciniki, $tnid, &$request, $cart) {
    ...
    if( isset($cart['customer']['emails'][0]['email']['address'])) {
        ...
        if( isset($sapos_settings['cart-cheque-submitted-message']) && $sapos_settings['cart-cheque-submitted-message'] != '' ) {
            $textmsg = $sapos_settings['cart-cheque-submitted-message'];
        }
...
/*        if( $invoice['shipping_status'] == 20 ) {
    ...
    //
    if( isset($settings['cart-cheque-submitted-emails']) && $settings['cart-cheque-submitted-emails'] != '' ) {
        $emails = explode(',', $settings['cart-cheque-submitted-emails']);
