...
// -------
//
function ciniki_sapos_wng_cartItemPaymentReceived(&$ciniki, $tnid, $request, $args) {

    //
    // Check that an item was specified
    //
    if( !isset($args['invoice_id']) || $args['invoice_id'] == '' ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.sapos.379', 'msg'=>'No cart specified'));
    }
    ...
    if( !isset($args['item_id']) || $args['item_id'] == '' ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.sapos.380', 'msg'=>'No item specified'));
    }
    ...
    if( !isset($rc['item']) ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.sapos.381', 'msg'=>'Unable to locate the invoice item'));
    }
    $item = $rc['item'];
