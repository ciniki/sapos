...
// -------
//
function ciniki_sapos_cartMerge(&$ciniki, $tnid, $primary_invoice_id, $secondary_invoice_id) {
    ...
    $strsql = "SELECT COUNT(id) AS num_shipments "
        ...
        . "WHERE invoice_id = '" . ciniki_core_dbQuote($ciniki, $secondary_invoice_id) . "' "
    ...
    if( isset($rc['shipments']) && $rc['shipments']['num_shipments'] > 0 ) {
        ...
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.sapos.514', 'msg'=>"Unable to remove, you have " . $n . " shipment" . ($n>1?'s':'') ."."));
    }
    ...
    $rc = ciniki_sapos_invoiceLoad($ciniki, $tnid, $secondary_invoice_id);
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    $invoice = $rc['invoice'];
    ...
    if( $invoice['invoice_type'] != 20 ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.sapos.515', 'msg'=>'Invoice is not a shopping cart and cannot be merged.'));
    }

    if( $invoice['status'] != 10 ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.sapos.516', 'msg'=>'This invoice is already recorded in the accounting system, and cannot be removed.'));
    }

    if( isset($invoice['transaction']) && count($invoice['transactions']) > 0 ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.sapos.517', 'msg'=>'There are transactions recorded for this invoice, it cannot be merged.'));
    }
    ...
    if( isset($invoice['items']) && count($invoice['items']) > 0 ) {
        foreach($invoice['items'] as $iid => $item) {
            ...
            if( $item['object'] != '' && $item['object_id'] != '' ) {
                ...
                $rc = ciniki_core_loadMethod($ciniki, $pkg, $mod, 'sapos', 'itemInvoiceMerge');
                if( $rc['stat'] == 'ok' ) {
                    $fn = $rc['function_call'];
                    $rc = $fn($ciniki, $tnid, $item, $primary_invoice_id, $secondary_invoice_id);
                    if( $rc['stat'] != 'ok' ) {
                        return $rc;
                    }
                }
            ...
            $rc = ciniki_core_objectUpdate($ciniki, $tnid, 'ciniki.sapos.invoice_item', $item['id'], [
    ...
    // Remove the invoice
    //
    $rc = ciniki_core_objectDelete($ciniki, $tnid, 'ciniki.sapos.invoice', $secondary_invoice_id, NULL, 0x04);
    if( $rc['stat'] != 'ok' ) {
        ciniki_core_dbTransactionRollback($ciniki, 'ciniki.sapos');
        return $rc;
    }
