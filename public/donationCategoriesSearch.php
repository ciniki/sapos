...
function ciniki_sapos_donationCategoriesSearch(&$ciniki) {
    ...
    //
    $strsql = "SELECT DISTINCT donation_category AS category "
        . "FROM ciniki_sapos_invoice_items "
        . "WHERE tnid = '" . ciniki_core_dbQuote($ciniki, $args['tnid']) . "' "
        . "AND donation_category <> '' "
        . "";
    if( isset($args['start_needle']) && $args['start_needle'] != '' ) {
        $strsql .= "AND (donation_category like '%" . ciniki_core_dbQuote($ciniki, $args['start_needle']) . "%' "
            . "OR donation_category like '% " . ciniki_core_dbQuote($ciniki, $args['start_needle']) . "%' "
            . ") ";
    }
