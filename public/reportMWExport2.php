...
// -------
//
function ciniki_sapos_reportMWExport2(&$ciniki) {
    //  
    ...
    $rc = ciniki_sapos_checkAccess($ciniki, $args['business_id'], 'ciniki.sapos.reportMWExport2'); 
    if( $rc['stat'] != 'ok' ) { 
        return $rc;
    }
	...
	$strsql = "SELECT ciniki_sapos_shipments.id, "
		...
		. "ciniki_sapos_shipment_items.id AS shipment_item_id, "
...
//		print_r($item);
		...
		if( !isset($invoices[$item['item']['invoice_id']]) ) {
			$invoices[$item['item']['invoice_id']] = array(
		...
		if( isset($items[$iid]['item']['shipment_quantity']) && $items[$iid]['item']['shipment_quantity'] != '' ) {
			...
			if( ($item['item']['flags']&0x4000) == 0 ) {
				...
				$invoices[$item['item']['invoice_id']]['nopromo_total_amount'] = bcadd($invoices[$item['item']['invoice_id']]['nopromo_total_amount'], $rc['total'], 4);
