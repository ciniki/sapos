...
function ciniki_sapos_expenseCategoryAdd(&$ciniki) {
    ...
    $rc = ciniki_sapos_checkAccess($ciniki, $args['tnid'], 'ciniki.sapos.expenseCategoryAdd'); 
    if( $rc['stat'] != 'ok' ) { 
        return $rc;
    }
    ...
    // Add the item
    //
    $rc = ciniki_core_objectAdd($ciniki, $args['tnid'], 'ciniki.sapos.expense_category', $args, 0x04);
    if( $rc['stat'] != 'ok' ) {
    ...
    $rc = ciniki_sapos_expenseUpdateCategorySequences($ciniki, $args['tnid'], $category_id, $args['sequence'], -1);
    if( $rc['stat'] != 'ok' ) {
        ciniki_core_dbTransactionRollback($ciniki, 'ciniki.sapos');
        return $rc;
    }
