...
// -------
//
function ciniki_sapos_invoiceCategories(&$ciniki) {
    //  
    ...
    if( $rc['stat'] != 'ok' ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.sapos.424', 'msg'=>'Unable to load settings', 'err'=>$rc['err']));
    }
    ...
    if( isset($rsp['stats']['categories']) ) {
        foreach($rsp['stats']['categories'] as $cid => $c) {
    ...
    $strsql .= "ORDER BY ciniki_sapos_invoices.invoice_date ASC, ciniki_sapos_invoices.invoice_number ASC, ciniki_sapos_invoice_items.category ";
