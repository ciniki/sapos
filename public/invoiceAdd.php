...
function ciniki_sapos_invoiceAdd(&$ciniki) {
    ...
    //
    if( ($ciniki['tenant']['modules']['ciniki.sapos']['flags']&0x40800200) > 0 ) {
...
//    }
    ...
    if( isset($args['objects']) && is_array($args['objects']) && count($args['objects']) > 0 ) {
        ...
        $rc = ciniki_sapos_lookupObjects($ciniki, $args['tnid'], $args['objects'], (isset($args['customer_id']) ? $args['customer_id'] : 0));
        if( $rc['stat'] != 'ok' ) {
    ...
    foreach($invoice_items as $i => $item) {
        ...
        if( isset($settings['invoice-autocat-desc-' . $item['description']]) 
            && (!isset($item['category']) || $item['category'] == '') 
            ) {
